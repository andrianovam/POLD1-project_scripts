...
(ggplot(aes(y=value, x=factor(Samples,levels = samples_levels), fill = factor(variable, levels = sigs_levels)), data = result_df)
  ...
  + theme_bw() 
  + scale_fill_manual(values = c("grey", "darkred", "blue", "darkblue","darkseagreen1", "darkseagreen2", "darkseagreen3"), name="",labels = c("SBS58\n", "SBS10c\n(POLD1 deficiency)", "SBS1\n", "SBS5\n", "SBS7a\n(UV irradiation)", "SBS7b\nU(UV irradiation)", "SBS7d\n(UV irradiation)"))
  + xlab("")
  + ylab("")
  + theme(axis.text=element_text(size=8),axis.title=element_text(size=10),legend.text=element_text(size=8))
  + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  + theme(strip.text = element_text(size = 10))
  + theme(legend.key.size = unit(0.4, "cm"),legend.spacing.y = unit(1.5, 'cm'))
)
...
tiff(filename="Output_plots/Figure2b_mutrate_fibros.tiff", width=7, height=5, res=300, units='cm')
...
(ggplot(aes(y=value, x=factor(Sample,levels = samples_levels), fill = factor(variable, levels = sigs_levels)), data = result_df)
  ...
  + theme_bw() 
  + scale_fill_manual(values = c("darkred","bisque1","bisque3","darkgrey","beige","darkblue"), name="")
  + xlab("")
  + ylab("")
  + theme(axis.text=element_text(size=8),axis.title=element_text(size=10),legend.text=element_text(size=8))
...
tiff(filename="Output_plots/Figure2d_fibros_mutrate_by_signature.tiff", width=6, height=6, res=300, units='cm')
